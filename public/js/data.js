function Data(ticker) {
  ...
  this.parseData = function(json) {
    try {
      ...
      // if the range between the high and low is less than 5, the labels increase by 1
      if (this.highest - this.lowest < 5) {
        ...
        this.priceRange = [Math.floor(this.lowest - 1), Math.floor(this.lowest + 6)];
      }

      // if the range between the high and low is more than 5, the labels increase by a percentage of its range
      else {
        ...
        this.priceRange = [Math.floor(this.lowest - (rangeDiv)), Math.floor(this.lowest + (rangeDiv * 6))];
      }
    } catch (err) {
      console.log(err);
      menuDiv.remove();
